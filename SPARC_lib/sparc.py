...
def two_condition_output(data, cond, xs, bars, outputter):
    """For each element of bars, creates an object generated by applying outputter() for each n in data
    to a sequence of values for which cond(n, x, bar) is true."""
    return [[outputter((n for n in data if cond(n, x, bar))) for x in xs] for bar in bars]


def two_condition_values(data, cond, xs, bars):
    """For each element of bars, creates a list of values for each n in data for which cond(n, x, bar) is true."""
    return two_condition_output(data, cond, xs, bars, lambda ns: list(ns))
...
def two_condition_counts(data, cond, xs, bars):
    ...
    return two_condition_output(data, cond, xs, bars, lambda ns: sum(1 for n in ns))
...
def line_plot(xs, y_lines, x_label, y_label, line_labels, ymin, ymax, colors=None, figsize=(10,3),
...
def conditional_line_plot(data, x_label, xs, line_label, lines, post_label, prior, posterior,
                          ...
                          legend_loc='upper left', ymin=0, ymax=100, min_any_x=0, min_all_x=0, min_any_bar=0,
    ...
    probs = [[y_labeler(r) if r.defined() else 0.0 for r in rs] for rs in ratios]
    line_plot(x_labels, probs, x_label, line_label, line_labels, ymin, ymax, colors=colors, figsize=figsize,
...
def make_box_plot(data, columns, column_cond, selector, x_label, y_label,
    ...
    selector is a function that takes a data element, and returns the value used
    to represent that item on the y axis."""
    box_collections = []
    for col in columns:
...
def zipped_sorted_averages(data, value_getter, labels_from, label_matcher):
    """For each label generated by a call to labels_from(row) for a row from data, this function
    finds the average of each call to value_getter(row) for each row that is true for
    label_matcher(row, label).

    It then returns a list of pairs of label and average, sorted in descending order of average."""
    labels = set()
    for row in data:
...
def sorted_average_plot(data, x_label, y_label, value_getter, labels_from, label_matcher, figsize=(10, 3)):
    """Creates bar chart and returns a markdown table of data from zipped_sorted_averages()."""
...
def grouped_markdown_table(nested_data, x_label, y_label, x_labels, bar_label, bar_labels,
    ...
    headers = [f'<br>{bar_label}', f'{x_label}<br>{x_labels[0]}'] + [f'<br>{x}' for x in x_labels[1:]]
    return f'## {y_label}\n\n{make_markdown_table(headers, table_data)}'
...
def average(nums):
    """Returns the average of the values in nums. If the average is undefined, returns None."""
    avg = Averager(0, 0)
    for n in nums:
        avg.add(n)
    return avg.average()
...
"""
    ...
    def test_2_condition(self):
        data = [i+1 for i in range(20)]
        xs = [(i + 1) * 2 for i in range(3)]
        bars = [(i + 1) * 3 for i in range(3)]
        cond = lambda n, x, b: n % x == 0 and n % b == 0
        values = two_condition_values(data, cond, xs, bars)
